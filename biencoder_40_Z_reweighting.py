...
def _train(train_loader, model, gloss_dict, synset_weight_dict, sense_weight_dict, optim, schedule, criterion,\
           gloss_bsz=-1, max_grad_norm=1.0, multigpu=False, silent=False, train_steps=-1):
    model.train()
    total_loss = 0.
    ...
    for i, (context_ids, context_attn_mask, context_output_mask, example_keys, _, labels) in train_data:
        ...
        loss = criterion(output, label_tensor) * synset_weight_dict[key]
...
def train_model(args):
    ...
    synset_weight_dict = json.load(open('/home/ysuay/codes/WSD-margin/preprocess/semcor_synset_weight_top_v11.json'))
    sense_weight_dict = json.load(open('/home/ysuay/codes/WSD-margin/preprocess/semcor_sense_weight.json'))
    ...
    for epoch in range(1, epochs+1):
        train_steps = -1
        model, optimizer, schedule, train_loss = _train(train_loader, model, train_gloss_dict, synset_weight_dict, sense_weight_dict, optimizer, schedule, criterion, gloss_bsz=args.gloss_bsz, \
...
if __name__ == "__main__":
	...
	choices=['semeval2007', 'senseval2', 'senseval3', 'semeval2013', 'semeval2015', 'ALL', 'all-test', 'meta'],
