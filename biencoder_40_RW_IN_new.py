...
def _train(train_loader, model, gloss_dict, sense_gloss_numlists, optim, schedule, criterion_old, \
               epoch, gloss_bsz=-1, max_grad_norm=1.0, multigpu=False, silent=False, train_steps=-1):
    model.train()
    total_loss = 0.
...
def train_model(args):
    ...
    for epoch in range(1, epochs+1):
        ...
        model, optimizer, schedule, train_loss = _train(train_loader, model, train_gloss_dict, \
            sense_gloss_numlists, optimizer, schedule, criterion, epoch, gloss_bsz=args.gloss_bsz, \
